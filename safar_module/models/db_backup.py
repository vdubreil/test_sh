...
class DbBackup(models.Model):
    ...
    send_mail_sftp_fail = fields.Boolean('Auto. E-mail on backup fail',
                                         help='If you check this option you can choose to automaticaly get e-mailed '
                    ...
                    for f in os.listdir(dir):
                        if rec.name in f:
                            ...
                            if os.path.isfile(fullpath):
                                try:
                                    # old_file = '%s_%s' % ('old', f)
                                    # sftp.rename(os.path.join(path_to_write_to,f), os.path.join(path_to_write_to, old_file))
                                    sftp.remove(os.path.join(path_to_write_to, f))

                                except Exception as error:
                                    _logger.critical('Error when trying to remove old file! Error: ' + str(error))
