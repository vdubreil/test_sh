...
class WooWebhookEpt(models.Model):
    ...
    topic = fields.Selection([("order.created", "When Order is Created"),
                              ("order.updated", "When Order is Created/Updated"),
    ...
    def _onchange_topic(self):
        """
        It is used to prevent create order/product/customer webhook.
        @author: Haresh Mori on Date 08-Feb-2021.
        """
        for record in self:
            if record.topic in ['order.created', 'product.created', 'customer.created']:
                raise Warning("Please select Create/Update Action for Order, products, and customers create and update")
    ...
    def create(self, vals):
        ...
        """
        available_webhook = self.search_read(
            [("topic", "=", vals.get("topic")), ("instance_id", "=", vals.get("instance_id"))], ["id"])
        if available_webhook:
            raise Warning(
                "Webhook already exists for selected action. You can't create webhook with same action.\nAction must be unique for the Instance.")
    ...
    def unlink(self):
        ...
        if webhook_ids:
            ...
            try:
                ...
                raise Warning("Something went wrong while deleting webhooks.\n\nPlease Check your Connection and "
    ...
    def get_webhook(self):
        ...
        if self.woo_id:
            ...
            try:
                ...
                raise Warning("Something went wrong while getting webhooks.\n\nPlease Check your Connection and "
                              "Instance Configuration.\n\n" + str(error))
        ...
        try:
            ...
            raise Warning("Something went wrong while creating webhooks.\n\nPlease Check your Connection and Instance "
                          "Configuration.\n\n" + str(e))

        if response.status_code in [200, 201]:
            new_webhook = response.json()
            self.write(
                {"woo_id": new_webhook.get("id"), "status": new_webhook.get("status"), "delivery_url": delivery_url})
