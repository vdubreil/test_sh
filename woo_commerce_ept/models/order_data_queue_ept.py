...
class WooOrderDataQueueEpt(models.Model):
    ...
    _description = "Woo Order Data Queue EPT"
    ...
    state = fields.Selection([("draft", "Draft"), ("partial", "Partially Done"),
                             ...
                             copy=False, default="draft", compute="_compute_state", store=True)
    order_data_queue_line_ids = fields.One2many("woo.order.data.queue.line.ept",
                                                "order_data_queue_id")
    ...
    def create_woo_data_queue_lines(self, orders):
        ...
        if vals_list:
            return self.env["woo.order.data.queue.line.ept"].create(vals_list)
        return False
